...
for i,num in enumerate(num_list):
    ...
    plt.legend()
    plt.title('Amplitude-accuracy lead time={} years'.format((101-num)*2),fontsize=30)
    fig.savefig(outdir + '/experiment2/amplitude_leadtime_{}.png'.format((101-num)*2))
    fig.clf()
    plt.close()
...
for j,err in enumerate(err_list):
    ...
    for i,num in enumerate(num_list):
        ...
        ax.plot([0,1,2,3,4],[acc0,acc1,acc2,acc3,acc4],"-",label='lead time={} years'.format((101-num)*2),marker='o',markersize=8,color=colorlist[i])
    plt.legend()
    plt.title('Amplitude-accuracy Err={}%'.format(err),fontsize=30)
    fig.savefig(outdir + '/experiment2/figure3_err_{}.png'.format(err))
    fig.clf()
    plt.close()
